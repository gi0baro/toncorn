...
class TestHTTP2Protocol:
    """Implementation-agnostic HTTP/2 behaviour, exercised against every HTTP/2 protocol
    over a real connection, with httpunk's HTTP/2 client (httpx only speaks HTTP/2
    through the `h2` package, which is not a dependency)."""

    pytestmark = skip_if_no_httpunk

    async def test_get_request(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
    ...
    async def test_request_scope(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
    ...
    async def test_post_request_body(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
            body = b""
            more_body = True
            while more_body:
    ...
    async def test_streaming_response(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
            await send({"type": "http.response.start", "status": 200, "headers": []})
    ...
    async def test_destreamed_response(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
    ...
    async def test_streaming_response_backpressure(
        self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int
    ):
    ...
    async def test_client_disconnect_mid_stream(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
    ...
    async def test_concurrent_streams(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
            http_scope = cast("HTTPScope", scope)
            await send({"type": "http.response.start", "status": 200, "headers": []})
    ...
    async def test_app_exception_returns_500(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
    ...
    async def test_no_response_returns_500(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
            return
    ...
    async def test_reset_contextvars(self, http2_protocol_cls: type[asyncio.Protocol], unused_tcp_port: int):
...
class TestZttp:
    """zttp-specific wire behaviour, driven through zttp's own sans-io client."""

    pytestmark = skip_if_no_zttp_h2

    async def test_head_request_has_no_body(self):
    ...
    async def test_204_response_has_no_body(self):
    ...
    async def test_partial_response_resets_stream(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
            await send({"type": "http.response.start", "status": 200, "headers": []})
    ...
    async def test_partial_response_after_transport_close_is_dropped(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
            await send({"type": "http.response.start", "status": 200, "headers": []})
    ...
    async def test_response_shorter_than_content_length_resets_stream(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
    ...
    async def test_response_longer_than_content_length_resets_stream(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
    ...
    async def test_rst_stream_disconnects_the_app(self):
    ...
    async def test_connection_lost_disconnects_the_app(self):
        received_disconnect = False
    ...
    async def test_keep_alive_timeout_closes_idle_connection(self):
    ...
    async def test_idle_frames_rearm_keep_alive_timer(self):
        app = Response("Hello, world", media_type="text/plain")
        protocol = get_connected_protocol(app, timeout_keep_alive=5)
        client = H2Client()
    ...
    async def test_shutdown_when_idle_closes_connection(self):
        app = Response("Hello, world", media_type="text/plain")
        protocol = get_connected_protocol(app)
        client = H2Client()
    ...
    async def test_shutdown_twice_is_a_no_op(self):
        app = Response("Hello, world", media_type="text/plain")
        protocol = get_connected_protocol(app)
    ...
    async def test_shutdown_refuses_new_streams_and_closes_after_last_response(self):
    ...
    async def test_goaway_closes_idle_connection(self):
        app = Response("Hello, world", media_type="text/plain")
        protocol = get_connected_protocol(app)
        client = H2Client()
    ...
    async def test_invalid_frames_close_the_connection(self, caplog: pytest.LogCaptureFixture):
        app = Response("Hello, world", media_type="text/plain")
        protocol = get_connected_protocol(app)
    ...
    async def test_resume_reading_waits_for_other_buffered_streams(self):
    ...
    async def test_early_response_ignores_late_request_frames(self):
    ...
    async def test_window_update_flushes_pending_response_data(self):
    ...
    async def test_app_returning_value_resets_stream(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
    ...
    async def test_send_after_rst_stream_is_dropped(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
            message = await receive()
    ...
    async def test_response_body_before_start_returns_500(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
    ...
    async def test_unexpected_message_after_start_resets_stream(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
            await send({"type": "http.response.start", "status": 200, "headers": []})
            await send({"type": "http.response.start", "status": 200, "headers": []})
    ...
    async def test_unexpected_message_after_completion_resets_stream(self):
        async def app(scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable):
            response = Response("Hello, world", media_type="text/plain")
            await response(scope, receive, send)
    ...
    async def test_eof_received_is_a_no_op(self):
        app = Response("Hello, world", media_type="text/plain")
        protocol = get_connected_protocol(app)
        assert protocol.eof_received() is None

    async def test_trace_logging(self, caplog: pytest.LogCaptureFixture, logging_config: dict[str, Any]):
        app = Response("Hello, world", media_type="text/plain")
    ...
    async def test_prior_knowledge_preface_selects_http2(self):
        app = Response("Hello, world", media_type="text/plain")
    ...
    async def test_prior_knowledge_preface_split_across_packets(self):
        app = Response("Hello, world", media_type="text/plain")
        negotiator, transport, _ = get_negotiator(app)
        client = H2Client()
    ...
    async def test_http1_request_selects_http1(self):
        app = Response("Hello, world", media_type="text/plain")
    ...
    async def test_tls_without_alpn_selects_http1(self):
        app = Response("Hello, world", media_type="text/plain")
    ...
    async def test_negotiator_times_out_silent_connection(self):
        app = Response("Hello, world", media_type="text/plain")
        negotiator, transport, loop = get_negotiator(app)
    ...
    async def test_negotiator_shutdown_closes_connection(self):
        app = Response("Hello, world", media_type="text/plain")
        negotiator, transport, _ = get_negotiator(app)
...
class TestHTTPunk:
    """httpunk-specific corners that cannot be expressed against a generic protocol."""

    pytestmark = skip_if_no_httpunk

    async def test_start_only_response(self, unused_tcp_port: int):
    ...
    async def test_body_handoff_abort_releases_parked_consumer(self):
    ...
    async def test_body_handoff_abort_releases_parked_producer(self):
    ...
    def test_is_ws_upgrade(self, headers: dict[str, bytes], expected: bool):
    ...
    async def test_access_log(self, unused_tcp_port: int):
    ...
    async def test_keepalive_and_total_requests(self, unused_tcp_port: int):
    ...
    async def test_h1_post_request_body(self, unused_tcp_port: int):
    ...
    async def test_h1_streaming_response(self, unused_tcp_port: int):
    ...
    async def test_h1_app_exception_returns_500(self, unused_tcp_port: int):
        from uvicorn.protocols.http.httpunk_impl import HTTPunkH1Protocol
    ...
    async def test_auto_protocol_serves_h1_and_h2(self, unused_tcp_port: int):
