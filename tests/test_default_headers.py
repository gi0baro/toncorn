...
async def test_default_default_headers(unused_tcp_port: int):
    config = Config(app=app, limit_max_requests=1, port=unused_tcp_port)
    async with run_server(config):
        with httpx.Client() as client:
            response = client.get(f"http://127.0.0.1:{unused_tcp_port}")
...
async def test_override_server_header(unused_tcp_port: int):
    ...
    config = Config(app=app, limit_max_requests=1, headers=headers, port=unused_tcp_port)
    async with run_server(config):
        with httpx.Client() as client:
            response = client.get(f"http://127.0.0.1:{unused_tcp_port}")
...
async def test_disable_default_server_header(unused_tcp_port: int):
    config = Config(app=app, limit_max_requests=1, server_header=False, port=unused_tcp_port)
    async with run_server(config):
        with httpx.Client() as client:
            response = client.get(f"http://127.0.0.1:{unused_tcp_port}")
...
async def test_disable_default_date_header(unused_tcp_port: int):
    config = Config(app=app, limit_max_requests=1, date_header=False, port=unused_tcp_port)
    async with run_server(config):
        with httpx.Client() as client:
            response = client.get(f"http://127.0.0.1:{unused_tcp_port}")
