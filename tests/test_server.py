...
def test_load_app_exits_with_startup_failure_on_unloadable_app() -> None:
    """An unloadable app exits with the dedicated startup-failure code.

    Process managers rely on `STARTUP_FAILURE` to tell a server that can never
    boot apart from one crashing at runtime.
